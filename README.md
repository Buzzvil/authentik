...
Buzzvil 운영에 필요한 **두 기능 패치**만 얹은 것이다. 그 외 코드는 upstream 과 동일하다.

빌드 이미지: **`ghcr.io/buzzvil/authentik:2026.2.7-cors-relaystate`** (multi-arch amd64/arm64, public)
소비처: `Buzzvil/buzz-k8s-resources` → `argo-cd/buzzvil-eks-ops/apps/authentik.yaml` 의 `global.image` override
(ops 클러스터 `ops-k8s-authentik`). base: `ghcr.io/goauthentik/server:2026.2.7`.

## 무슨 패치인가

### AWS WorkSpaces SAML RelayState
...
### OAuth2 CORS 정규식 redirect URI

OAuth2 provider의 redirect URI가 `REGEX` matching mode일 때 `/authorize/`뿐 아니라
`/token/`, `/userinfo/`, provider-info 응답의 CORS 검사에서도 같은 정규식 규칙을 사용한다.
정규식 redirect URI를 사용하는 브라우저 앱이 인가 후 토큰 요청에서 CORS로 차단되는 문제를
해결한다. malformed regex는 요청을 500으로 만들지 않고 로그를 남긴 뒤 건너뛴다.

## 빌드 / 릴리스

`.buzzvil/Dockerfile` 이 공식 이미지 위에 패치된 런타임 파일만 COPY 하는 overlay 이고, 빌드 워크플로우
(`build-patched-ghcr`)가 **`<ver>-cors-relaystate` 태그 push** 시 GitHub-hosted 러너에서 multi-arch(amd64/arm64)
...
authentik 을 새 버전으로 올릴 때마다 두 기능 패치를 새 버전 기준으로 다시 적용한다:
1. upstream 태그(`version/<X.Y.Z>`)에서 브랜치 `buzzvil/workspaces-relaystate-<X.Y.Z>` 생성
2. SAML 및 OAuth2 CORS 패치 재적용, `.buzzvil/Dockerfile` 의 `FROM ...:<X.Y.Z>` 갱신
3. 그 커밋에 태그 **`<X.Y.Z>-cors-relaystate`** push → 워크플로우가 멀티아치 이미지 빌드 + 동일 태그 Release 생성
4. ops(buzz-k8s-resources) 의 `global.image` tag 를 `<X.Y.Z>-cors-relaystate` 로 갱신 → WorkSpaces 로그인과 CORS 재검증
